 ...
 *************************************************************/ 
(function ($){
	var xOffset = 5; // x 偏移像素
    var yOffset = 20; // y 偏移像素 
	var common ={
	...
	validator.showTipMsg=function (element,msg,errindex,isSuccess){
		...
		var left=offset.left+element.width()+10;
		var validator_msg_ico;	
		validator.removeTipMsg(element); //移除错误信息
		if(isSuccess){
			element.removeClass(this['ErrorInput']);
			tipmsg="校验成功";
			validator_msg_ico="validator_msg_successico_tip";
		}else {
			element.addClass(this['ErrorInput']);
			validator_msg_ico="validator_msg_errorico_tip";
		}
		/* 此处为span追加展示方式
		jQuery("<span class='"+showTipCss.spaceID+"'><span class='"+showTipCss.typeBody+"'>&nbsp;&nbsp;&nbsp;</span></span>").html(showTipCss.msg).insertAfter(element);
		*/	
		/*此处为tip展示方式*/
		var mytip=	jQuery( '<div class="validator_msg_tip"><div class="validator_msg_container_tip"><div class="'+validator_msg_ico+'">'+
								   	'<div class="validator_tip_msg_content">'+tipmsg+
									'<div class="z">◆</div> <div class="y">◆</div>' +  
									'</div></div></div></div>');
